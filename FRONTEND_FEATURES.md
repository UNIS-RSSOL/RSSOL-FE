...
### UI 구현
- 직원 근무 가능 시간 등록/수정 페이지 (`src/pages/employee/calendarAdd/CalModEmp.jsx`)
  - 주간 캘린더 UI (`EmployeeScheduleCalendar` 컴포넌트)
  - 시간 슬롯 드래그 선택으로 근무 가능 시간 선택
  - 선택된 시간 슬롯 시각적 표시
  - 기존 availability 불러오기 및 초기 선택 상태 설정
  - 연속된 시간대 자동 병합
  - 최초 입력과 수정 모두 처리
