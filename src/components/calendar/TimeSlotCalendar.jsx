...
function TimeSlotCalendar({ onTimeSlotClick, getAvailabilityCount }) {
  ...
  const getCellStyle = (day, hour) => {
    ...
    // 가장 진한 색 #3370FF (51, 112, 255) 기준으로 단계별로 더 연하게
    // 레벨 4가 가장 진함 (alpha 1.0), 레벨 1이 가장 연함 (alpha 0.25)
    const alpha = 0.25 + (level / 4) * 0.75;

    return {
      backgroundColor: `rgba(51, 112, 255, ${alpha})`,
  ...
  return (
    ...
    >
      <div className="flex flex-shrink-0 flex-row w-full h-[35px]">
        <div className="flex-shrink-0 w-[36px] h-full" />
        {days.map((day) => (
          <div key={day}>
      ...
      <div className="flex-1 flex flex-col w-full">
        {hours.map((hour) => (
          ...
          >
            <div className="flex-shrink-0 w-[36px] h-full items-center justify-center flex">
